...
module.exports = function (connect) {
  ...
  function NeDBStore(options) {
    ...
    _this._defaultExpiry =
      typeof options.defaultExpiry === "number" && Number.isFinite(options.defaultExpiry) && options.defaultExpiry > 0
  ...
  NeDBStore.prototype.destroy = function (sessionId, callback) {
    // IMPORTANT: NeDB datastores auto-buffer their commands until the database is loaded
    this.datastore.remove({ _id: sessionId }, { multi: false }, function (err /*, numRemoved */) {
      return callback(err)
    })
  }
